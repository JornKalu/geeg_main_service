...
class Invite(Base):
    ...
    user_id = Column(BigInteger, nullable=True) # Null if the person isn't a user yet
...
def create_invite(db: Session, project_id: int, role_id: int, email: str, sent_by: int, full_name: str = None, message: str = None, user_id: int = None, status: str = 'pending', commit: bool = False):
    invite = Invite(
        user_id=user_id,
...
def get_invites(db: Session, filters: Dict = {}):
    query = db.query(Invite).filter(Invite.deleted_at == None)
...
def get_invite_by_email_and_project(db: Session, email: str, project_id: int):
    ...
    """
    return db.query(Invite).filter_by(
        email=email, 
        project_id=project_id, 
        deleted_at=None
    ).first()